...
//   return (
//     <div className="min-h-screen bg-gray-50 pt-[80px] md:pt-[90px] lg:pt-[100px]">
...
// Icons
const BuildingIcon = () => (
  <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-400 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth={1.6} viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" d="M3 21h18M3 7h18M3 13h18M9 3v4m6-4v4M9 13v4m6-4v4M9 7v2m6-2v2" />
  </svg>
);
...
// Career Card Component - Responsive
const CareerCard = ({ career, onCardClick }) => (
  <div
    onClick={() => onCardClick(career)}
    className={`flex flex-col w-full max-w-[320px] sm:max-w-[340px] md:max-w-[352px] cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${career.faded ? 'opacity-55' : 'opacity-100'} mx-auto`}
  >
    <div className="pb-2 sm:pb-3">
      <h3 className="text-lg sm:text-xl md:text-[22px] font-extrabold text-slate-900 leading-tight mb-1 sm:mb-2 tracking-tight">
        {career.title}
      </h3>
      <div className="flex items-center flex-wrap gap-x-3 sm:gap-x-4 gap-y-1">
        <span className="flex items-center gap-1 text-[11px] sm:text-[13px] text-slate-500">
          <BuildingIcon />
          {career.company}
        </span>
        <span className="flex items-center gap-1 text-[11px] sm:text-[13px] text-slate-500">
    ...
    <div className="relative w-full rounded-xl overflow-hidden" style={{ height: '160px', minHeight: '160px' }}>
      <img src={career.image} alt={career.title} className="absolute inset-0 w-full h-full object-cover" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
      <div className="absolute bottom-0 left-0 px-3 sm:px-4 pb-2 sm:pb-3">
        <p className="text-[9px] sm:text-[11px] font-semibold tracking-widest uppercase text-blue-400 mb-0.5 sm:mb-1">
          {career.subText}
        </p>
        <p className="text-sm sm:text-base md:text-[18px] font-bold text-white leading-snug">
          {career.description}
        </p>
      </div>
    </div>
...
// Job Detail Page Component - Responsive
const JobDetailPage = ({ job, onBack }) => {
  const [formData, setFormData] = useState({
    fullName: '',
    email: '',
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-[70px] sm:pt-[80px] md:pt-[90px] lg:pt-[100px]">
      {/* Header with Back Button */}
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <button onClick={onBack} className="flex items-center gap-1 sm:gap-2 text-gray-600 hover:text-purple-600 transition-colors text-sm sm:text-base">
            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
            Back to Careers
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 xl:gap-12">
          ...
          <div className="flex-1">
            ...
            <div className="mb-4 sm:mb-6">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">{job.title}</h1>
            ...
            {/* Share Job Button */}
            <button className="flex items-center gap-1 sm:gap-2 text-gray-500 hover:text-purple-600 transition-colors mb-6 sm:mb-8 text-sm sm:text-base">
              <ShareIcon />
              <span>Share Job</span>
            </button>
            ...
            {/* Info Bar - Responsive */}
            <div className="mb-6 sm:mb-8 border-y border-[#F1F5F9] py-4 sm:py-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                {/* Left Column */}
                <div className="space-y-4 sm:space-y-6">
                  <div className="flex flex-col gap-1">
                    <h4 className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-wider">SALARY</h4>
                    <p className="text-gray-800 font-medium text-sm sm:text-base">{job.jobDetails.salary}</p>
                  </div>
                  <div className="flex flex-col gap-1">
                    <h4 className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-wider">ROLE TYPE</h4>
                    <p className="text-gray-800 font-medium text-sm sm:text-base">{job.jobDetails.roleType}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4 sm:space-y-6">
                  <div className="flex flex-col gap-1">
                    <h4 className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-wider">EXPERIENCE</h4>
                    <p className="text-gray-800 font-medium text-sm sm:text-base">{job.jobDetails.experience}</p>
                  </div>
                  <div className="flex flex-col gap-1">
                    <h4 className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-wider">DEPARTMENT</h4>
                    <p className="text-gray-800 font-medium text-sm sm:text-base">{job.jobDetails.department}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6 sm:mb-8">
              ...
              <ul className="space-y-1.5 sm:space-y-2">
                {job.jobDetails.responsibilities.map((resp, idx) => (
                  <li key={idx} className="flex items-start gap-1.5 sm:gap-2 text-gray-600 text-sm sm:text-base">
            ...
            {/* Requirements */}
            <div className="mb-6 sm:mb-8">
              <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">Requirements</h2>
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {job.jobDetails.requirements.map((req, idx) => (
                  <span key={idx} className="px-2 sm:px-3 py-0.5 sm:py-1 bg-purple-100 text-purple-700 rounded-full text-xs sm:text-sm">
                    {req}
                  </span>
                ))}
              </div>
          ...
          <div className="w-full lg:w-[380px] xl:w-[400px]">
            <div className="bg-white rounded-xl p-5 sm:p-6 shadow-md sticky top-24">
              <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 sm:mb-6">Apply Now</h2>
              <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Full Name</label>
                  <input type="text" required value={formData.fullName} onChange={(e) => setFormData({...formData, fullName: e.target.value})}
                    className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition text-sm"
                    placeholder="Jane Doe" />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Portfolio Link</label>
                  <input type="url" required value={formData.portfolioLink} onChange={(e) => setFormData({...formData, portfolioLink: e.target.value})}
                    className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition text-sm"
                    placeholder="https://dribbble.com/jane" />
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Resume (PDF)</label>
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 sm:p-4 text-center hover:border-purple-400 transition-colors">
                    <input type="file" accept=".pdf" className="hidden" id="resume-upload" onChange={(e) => setFormData({...formData, resume: e.target.files[0]})} />
                    <label htmlFor="resume-upload" className="cursor-pointer">
                      <svg className="w-6 h-6 sm:w-8 sm:h-8 text-gray-400 mx-auto mb-1 sm:mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                      </svg>
                      <p className="text-gray-500 text-xs sm:text-sm">Upload a file</p>
                      <p className="text-gray-400 text-[10px] sm:text-xs">PDF up to 10MB</p>
                    </label>
                  </div>
                </div>
                <button type="submit" className="w-full py-2 sm:py-3 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition-all duration-200 text-sm sm:text-base">
                  Submit Application
                </button>
    ...
    <footer className="bg-white border-t border-gray-200 mt-12 sm:mt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div className="flex flex-col sm:flex-row md:flex-row items-center justify-between gap-4 sm:gap-6">
          ...
          <p className="text-gray-400 text-[10px] sm:text-xs md:text-[13px] text-center">© 2025 SimcuraRX. Leading the future of healthcare integration.</p>
